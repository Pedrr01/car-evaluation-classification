...
# --- CONFIGURAÇÃO PADRONIZADA PARA O MLP ---
ALGO_NAME = "Multi-Layer Perceptron (MLPClassifier)"
CLASSIFIER_CLASS = MLPClassifier
PARAM_COMBOS = [
    {"hidden_layer_sizes": (50,), "activation": "relu", "max_iter": 1000, "random_state": 42},
    {"hidden_layer_sizes": (100, 50), "activation": "relu", "max_iter": 1000, "random_state": 42},
    {"hidden_layer_sizes": (50,), "activation": "tanh", "max_iter": 1000, "random_state": 42},
]
...
def get_param_label(params):
    if 'hidden_layer_sizes' in params:
        return f"camadas={params['hidden_layer_sizes']}, ativ={params['activation']}"
    return str(params)


def load_and_preprocess_data(file_path=None):
    if file_path is None:
        # Tenta carregar pela URL direto da internet se falhar nos caminhos locais
    ...
    if file_path.startswith('http'):
        ...
        data_source = "real (car.data via URL)"
    elif os.path.exists(file_path):
        df = pd.read_csv(file_path, header=None, names=feature_names + [target_name])
        data_source = "real (car.data local)"
    else:
        # Fallback sintético do seu código base se o arquivo sumir completamente
...
def plot_results(results, cm, label_encoder, y_test, y_pred_test, algo_name, data_source):
    ...
    ax0.set_xticklabels(combo_labels, rotation=10, ha='right', fontsize=9)
...
def main():
    ...
    X, y, label_encoder, data_source = load_and_preprocess_data()
